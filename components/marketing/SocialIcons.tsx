...
// aria-hidden on every glyph: each of these is wrapped in an <a> that
// already carries the network name via aria-label, so an exposed unnamed
// <svg> beside it is pure noise in the accessibility tree.
type IconProps = { size?: number };

export function InstagramIcon({ size = 15 }: IconProps) {
  return (
    <svg width={size} height={size} viewBox="0 0 24 24" fill="none" aria-hidden stroke="currentColor" strokeWidth={1.75}>
